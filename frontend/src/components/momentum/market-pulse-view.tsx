...
const IntelNewsSkeleton = dynamic(() => import('@/components/momentum/intel-news-card').then(m => m.IntelNewsSkeleton), { ssr: false });
...
interface MarketPulseViewProps {
  data: DashboardData;
  tierLoading: { summary: boolean; signals: boolean; intel: boolean };
...
export function MarketPulseView({
  ...
  const intelImages = data.intel_images || [];
  const imagesLoading = tierLoading.intel;
  ...
  // Carousel state for Weekly Intelligence
  const [intelIndex, setIntelIndex] = useState(0);

  const handleNextIntel = useCallback(() => {
    if (intelImages.length === 0) return;
    setIntelIndex((prev) => (prev + 1) % intelImages.length);
  }, [intelImages.length]);

  const handlePrevIntel = useCallback(() => {
    if (intelImages.length === 0) return;
    setIntelIndex((prev) => (prev - 1 + intelImages.length) % intelImages.length);
  }, [intelImages.length]);
  ...
  const startAutoSlide = useCallback(() => {
    ...
    autoSlideRef.current = setInterval(() => {
      if (!isPaused) handleNextIntel();
    }, 10000); // 10 seconds
  }, [handleNextIntel, isPaused]);
  ...
  return (
    <div className="pt-4 md:pt-8 pb-12 max-w-7xl mx-auto space-y-6 font-sans">
      ...
      {/* ── Weekly Intelligence (AI Generated News) ── */}
      <div className="pt-4 border-t border-[#2A2A2A]">
        <SectionHeader
          icon="◆"
          iconColor="#22d3ee"
          title="Weekly Intelligence"
          description="Visual metaphors generated by our 3-step AI pipeline. We transform raw signals into branded 'Blueprint' and 'Tactile' imagery for the week's top conviction setups."
        />
        <DataReveal
          loading={imagesLoading}
          skeleton={<IntelNewsSkeleton />}
        >
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* Featured Column (2/3) */}
            <div className="lg:col-span-2">
              <div className="mb-4 flex items-center justify-between px-2">
                <h3 className="font-serif text-2xl text-[#e2b857]">Most Recent</h3>
                <div className="flex gap-2">
                   <button 
                     onClick={handlePrevIntel}
                     className="flex h-8 w-8 items-center justify-center rounded-full bg-white/5 text-white/40 transition-colors hover:bg-white/10 active:scale-95"
                   >
              ...
              {intelImages[intelIndex] ? (
                ...
                >
                  <AnimatePresence mode="wait">
                    <motion.div
                      key={intelImages[intelIndex].ticker}
                ...
                <div className="aspect-video flex items-center justify-center rounded-2xl border border-dashed border-[#2A2A2A]">
                   <p className="text-[#6B6B6B] font-mono-data text-xs uppercase tracking-widest">Featured intel pending...</p>
                </div>
              )}
            </div>
